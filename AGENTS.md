...
cargo test --workspace
# Always build all plugins (not just modified ones — cheapest to build them all):
for manifest in plugins/*/Cargo.toml; do
  cargo build --manifest-path "$manifest" --target wasm32-wasip1 --release
done
# On Windows (PowerShell):
# foreach ($m in (Get-ChildItem plugins/*/Cargo.toml)) { cargo build --manifest-path $m.FullName --target wasm32-wasip1 --release }
```
...
- **Run `cargo build --manifest-path plugins/<name>/Cargo.toml --target wasm32-wasip1 --release`** — native tests pass even when WASM-target-gated code has syntax errors; always build for WASM explicitly. When in doubt, build all plugins (see "Before Opening a PR").
