...
/// Checks: plugin.toml structure, tags alphabetical, author present, [config] matches source usage.
pub async fn lint(path: Option<&str>) -> Result<()> {
    ...
    if let Some(meta) = meta {
        ...
        // Check author is present (standardize on "Slate Community" for official plugins)
        if meta
            .get("author")
            .and_then(|v| v.as_str())
            .unwrap_or("")
            .is_empty()
        {
            println!("  ⚠ Missing author field (expected e.g. \"Slate Community\")");
            warnings += 1;
        } else {
            println!("  ✓ author present");
        }
    } else {
